...
def functionCalling(functionName:str,functionArgs:dict) -> ChatReply:
    if(functionName == "riseimaterials"):
        ...
        return ChatReply(ChatType.FUNCTION,content=CalculatorManager.riseimaterials_v2(targetEstimated,True,CalculateMode.SANITY,maxItems=5),plainText=f"{target}の周回ステージを表示します：")
        ...
        return ChatReply(ChatType.FUNCTION,content=CalculatorManager.riseistages_v2(targetEstimated,True,CalculateMode.SANITY,maxItems=5),plainText=f"ステージ{targetEstimated}の理性効率を表示します：")
